...
float MatrixMath::DotProduct(float* vector_one, int length , float* vector_two){
  ...
  //NOTE: Currently, there is no check implemented for equal-sizedness
  //Input Parameters:
  //  vector_one:  first input vector
  //  int size:    length of vector
  //  vector_two:  second input vector
  //Output Parameters:
  //  float out:	Dot product output
  float out=0.0;
...
void MatrixMath::NormalizeVector(float* MatIn, int n, int m, float* MatOut) {
  //This function normalizes a vector so that norm|MatIn|=1
  //This function also works on matrices (if there is such a thing as matrix normalization such that sqrt( a_11^2+...a_1n^2 + ... a_m1^2+a_mn^2)=1   )
  //Input Parameters:
